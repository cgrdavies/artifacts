...
5. Share the link and a short summary. Keep the source and receipt. Updated artifacts get new URLs; there is no edit/list/history API yet.
...
No-index is not authentication. Creation and deletion still use the existing unauthenticated API. Links may be copied. Keep sources locally because hosted copies expire after about 30 days. The original table schema is preserved: Markdoc uses the markdown type; HTML uses raw storage with its content type.
...
Before declaring a hosted update complete, verify capabilities version 2, no-index/referrer headers, ordinary Markdown, a diagram and a self-contained HTML interaction on the live domain. Local tests alone are not deployment proof.
