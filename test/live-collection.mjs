...
let browser;
try{
 const caps=await (await get(base+'/api/capabilities')).json();assert.equal(caps.collections.editTokenRequired,true);assert.equal(caps.theme.savedPreference,true);
 ...
 report.checks.push('live capabilities, no-index/no-referrer, exact source, no edit key in public metadata, unauthenticated edit denied');
