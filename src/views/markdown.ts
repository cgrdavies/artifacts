...
<body><main class="container${navigation ? ' collection' : ''}"><header class="site-header">${navigation ? `<a class="site-title" href="${escapeHtml(navigation.url)}">${escapeHtml(navigation.title)}</a>` : '<span class="site-title">Artifacts</span>'}<div class="theme-controls"><label for="theme-preference">Appearance</label><select id="theme-preference" aria-label="Color theme" disabled><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></div></header>${navigation ? `<div class="collection-layout"><aside class="collection-sidebar">${contents}</aside><div class="collection-main"><details class="collection-mobile"${navigation.currentUrl ? '' : ' open'}><summary>Collection <span class="page-position">${navigation.pages.length} pages</span></summary>${contents}</details><article class="document"${annotationAttrs}>${pageTools}${body}</article>${adjacent}</div></div>` : `<article class="document"${annotationAttrs}>${pageTools}${body}</article>`}</main></body>
</html>`;
}
